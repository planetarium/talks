...
<!-- 본격적으로 발표를 시작하기전에 간단히 저의 소개를 하자면 GitHub에서 Moreal이라는 닉네임으로 활동하고 있고 플라네타리움에서 오픈소스 소프트웨어 엔지니어로 개발하고 있습니다. 오픈소스 활동을 정말 좋아하는 사람 중 한명입니다. -->
...
<!-- 현재는 나인크로니클 이라는 이름으로 이름을 바꾸고 Unity 기반의 블록체인 RPG 게임으로 만들어 나가고 있습니다, 또한 이전과 다르게 직접 Libplanet이라는 이름의 블록체인 엔진을 만들어서 사용하고 있습니다. 중앙 서버 없이 게이머 간의 P2P 통신을 통해 게임 네트워크를 운영할 수 있는 네트워크, 스토리지 라이브러리인데요, P2P 네트워크에서 데이터의 영속성과 치트 방지등을 제공해야 되다보니 블록체인 기술을 구현할 필요가 있었습니다.  -->
...
<!-- 그리고 블록체인과 직렬화의 관계에 대해서 이야기해보고자 합니다. 블록체인에서는 안에서 다루는 모든 데이터에 암호학적 해시와 디지털 서명을 사용합니다. 그리고 이러한 암호학적 해시나 디지털 서명의 입력으로 사용할 평문을 얻기 위해서 직렬화를 하고 암호학적 해시와 디지털 서명의 결과는 데이터의 무결성을 검사하기 위한 용도로 사용됩니다. 그렇기에 해시 및 서명이 다른 경우 입력이 동등하지 않다는 가정을 세울 수 있는데 이 가정을 세울 수 없을 경우 항상 입력을 동봉하여 들고 다녀야 하고 계산을 줄이기 위해 사용하는 해시의 의미가 퇴색되기 때문에 좋지 못합니다. -->
...
<!-- 이전 슬라이드에서 말한 가정을 지키기 위해 블록체인에서 사용되는 직렬화 포맷은 의미와 표현의 관계가 1:1일 필요가 있습니다. 예를 들며 설명하기 위해 블록체인에서 사용되는 트랜잭션의 필드와 값을 가져와 테이블로 띄워 보았습니다.  -->
...
<!-- 이를 직렬화 포맷 중 가장 널리 사용되는 직렬화의 대명사 JSON으로 표현해보았습니다. 이전 슬라이드의 테이블에 있던 필드와 값들을 모두 그대로 가져왔고 보기 쉽게 indent를 넣고 순서 또한 동일하게 넣었습니다. 이렇게 직렬화 하였을 때의 값을 SHA1으로 해싱한 결과도 아래에 같이 표기했습니다. 결론부터 이야기하자면 저희는 JSON을 택할 수 없었습니다. 그 이유는 JSON은 같은 의미를 위 방식과 다르게 표현할 수도 있기 때문입니다. -->
...
<!-- 이번에는 순서는 동일하게 하는 대신 탭, 스페이스, 개행문자 같은 화이트스페이스 문자를 모두 없애보았습니다. 이 또한 JSON 문법에 맞는 표현 방식이고 파싱 또한 잘 될 것입니다. 하지만 이 역시 입력이 달라지기 때문에 SHA1 해시가 달라지는 것을 볼 수 있었습니다. -->
...
<!-- 저희가 원하는 가정, 해시와 서명이 다르면 입력도 다르다, 를 만족하는 직렬화 포맷은 Bijection, 즉 전단사 함수의 특성을 지닐 필요가 있었습니다. 이해를 돕기 위해 슬라이드에 한 이미지를 첨부했습니다. 한 의미에 대해서 한 표현만 대응하는, 1:1 관계를 만족해야만 했었습니다. -->
...
<!-- 반면에 JSON은 세 가지의 에를 들어가며 살펴보았듯이 한 의미에 대해서 여러가지 방법으로 표현될 수 있었습니다. 다시 말해 본 슬라이드의 그림과 같이 한 의미에 대해 N개의 표현이 존재한다는 것이죠. 때문에 JSON을 직렬화 포맷으로 채용하기는 어려웠습니다.-->
...
<!-- JSON 외에 pickle, BSON, msgpack과 같은 다른 직렬화 포맷들도 살펴보았지만 모두 입력과 출력의 관계가 1:N 이었습니다. 그 이유는 주로 딕셔너리에서 키 값의 순서를 스펙에서 정의하지 않거나 문자열 표현 방식등 표현방식을 제한하지 않아 여러 형태로 표현될 수 있기 때문이었습니다. -->
...
<!-- 그래서 이전에 있던 다른 사람들은 어떻게 했을지, Libplanet과 비슷한 문제를 푸는 프로젝트에는 무엇이 있을지 찾아보기로 했습니다. 그래서 잘 생각해보니, 등잔 및이 어둡다고, 우리가 많이 쓰고 있는 비트토렌트가 떠올랐습니다. P2P 분산 네트워크 기술을 사용하고, 받을 파일이 같은 파일인지 다른 파일인지를 해시를 이용해서 체크하는 등, 목표는 다르지만 Libplanet과 많이 겹치는 문제들을 이미 푼 프로젝트니까요. -->
...
<!-- 그러한 비트토렌트는 Bencoding이라는 직렬화 포맷을 사용했습니다. 비트토렌트에서 쓰기 위해서 만들어졌던 직렬화 포맷이었고 생긴 이유도 저희가 겪고 있던 문제와 같이 1:N 대응 문제 때문에 당시 많이 쓰이던 XML이나 pickle같은 직렬화 포맷을 사용할 수 없었기 때문이었습니다. 비트토렌트는 토렌트 메타데이터를 직렬화 할 때 Bencode를 통해 직렬화 하고 그 토렌트 파일을 해시하여 매그닛 주소로 사용합니다. 1:1 대응이 되는 덕분에 같은 파일은 같은 토렌트 파일 및 매그닛으로 표현되어 잘 겹치고 공유가 더 잘 되는 장점을 가지게 되었죠.  -->
...
<!-- Bencode는 아무래도 비트토렌트와 함께 2001년에 만들어진 직렬화 포맷이었기 때문에 시대적인 한계가 있었습니다. 우선 빈 값, None을 지원하지 않았고 True나 False같은 bool 논리 값 또한 지원하지 않았습니다. 그리고 문자열은 모두 바이트 배열이었고 유니코드를 쓰기 위해서는 알아서 인코딩 해서 사용해야 했었습니다. 하지만 그냥 그렇게 된 것이 아니라 파이썬에 bool 타입이 들어온 것이 2002년 부터였고 때는 파이썬 3가 나오기 이전이라 기본 문자열이 바이트 배열이었기 때문이었습니다. 말 그대로 시대적 한계인 것이죠. -->
...
<!-- 그래서 Bencode를 확장한 직렬화 포맷을 정의하기로 했습니다. 확장하였기 때문에 Bencodex라고 이름을 붙였고 전 슬라이드에서 언급했던 Bencode에서 있던 시대적 한계를 해결하였습니다. 그리고 Bencode로 인코딩된 값을 Bencodex로도 디코딩할 수 있게끔 하위호완성도 지켰고요. -->
...
<!-- 그리고 비트토렌트에서 첫 구현체를 파이썬으로 만들었던 것 처럼, 파이썬으로 Bencodex의 첫 구현체를 만들어 보기로 했습니다. 이 부분에서는 구현 특징에 대해 이야기하고 그렇다면 Bencodex는 어떻게 1:1 대응을 보장하는 지, 그리고 스펙 구현 테스팅과 활용등에 대해서 이야기 해보고자 합니다. -->
...
<!-- 우선 다행히도 Bencodex의 모든 자료형은 파이썬에서 None, bool, int, bytes, list, dict 같은 빌트인 자료형들만으로 표현이 가능했습니다. 슬라이드에 테이블로 정의해 놓은 것 처럼 말이죠. -->
...
<!-- 또한 파이썬에는 marshal, pickle, json 등 표준 라이브러리에 포함된 직렬화 모듈에서 사용되는 사실상의 표준(de facto standard) 인터페이스가 있습니다. dump와 load가 그런 것입니다. 그렇기 때문에 bencodex-python도 이를 따라 구현하였고 사용자 입장에서는 보다 친숙한 경험을 얻을 수 있을 것입니다. -->
...
<!-- 또한 PEP 484의 typing, 타입 힌트를 지원하고 mypy 타입 체커를 통해 런타임 이전에 오류를 사전 방지할 수 있도록 하였습니다. -->
...
<!-- 예를 들어 위와 같은 코드를 작성하고 mypy로 타입 검사를 하게되면 주석으로 달아놓은 것과 같은 에러 메시지와 함께 잘못된 순서로 넣었음을 사전에 알 수 있습니다. 만약 타입힌트를 추가하지 않았다면 이는 어려웠을 것이고 런타임이 되어서야 실수를 알아챌 것입니다. -->
...
<!--  그리고 타입 검사에서 파이썬 자료형 가운데 Bencodex로 부호화가 가능한 것들만 허용하는 타입, BValue를 지원하려고 하였지만 -->
...
<!-- 타입스크립트 구현체를 이용해서 Bencodex로 인코딩 된 바이너리 파일을 웹브라우저에서 볼 수도 있게 해놓았습니다. 그래서 실제 게임에서 운용중인 블록체인 혹은 상태 데이터를 볼 때 자주 사용하곤 합니다. 또한 이 프로젝트도 AGPL로 배포된 오픈소스 프로젝트이고 이 발표를 듣고 관심이 있으신 분들은 디자인 개선이나 기능 추가 등에 기여하실수도 있습니다! -->
...
<!-- 또한 Bencodex로 깃허브에서 검색해보면 관련된 구현체 및 프로젝트들을 찾아볼 수 있습니다. 참고로 세 번째에 나오는 프로젝트는 Bencodex가 아닌 Bencoding과 관련된 프로젝트입니다. -->
...
<!-- 또한 라이브러리 구현체들을 스펙을 관리하는 리포지토리에서 LIBRARIES.tsv 라는 파일에서 관리하고 있어요. 실제로 본 언어에 붙여서 사용할 수 있을지 찾아보려 할 때 이전 슬라이드 같이 깃허브에서 검색해서 찾아볼 수도 있지만 이렇게 한 파일에서 정리해놓으면 찾기 더 수월하니깐요. 취미로 사용하고 있는 언어로 구현해보기에도 난이도도 그렇게 높지 않아서 흥미가 생기시는 분들은 사이드 프로젝트로 시도해보셔도 좋을 것 같습니다. -->
...
<!-- 지금까지 발표 들어주셔서 감사합니다. 이번 파이콘 발표는 사전녹화로 진행되기 때문에 본 발표와 관련된 질문을 하시고 싶으신 분들께 라이브로 바로 받기는 어려울 것 같아 제 트위터 계정이나 플라네타리움 디스코드 developer 채널에 놀러오셔서 질문 남겨주시면 답변해드리도록 하겠습니다. 감사합니다. -->
